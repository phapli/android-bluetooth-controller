...
public class DeviceControlActivity extends Activity {
    ...
    private BluetoothGattCharacteristic accelerationEnableCharacteristic;

    private final String LIST_NAME = "NAME";
    private final String LIST_UUID = "UUID";

    private byte[] accelerationEnable = null;
...
//        mGattServicesList.setOnChildClickListener(servicesListClickListner);
        ...
        mToggle = (ToggleButton) findViewById(R.id.led_toogle);
        mToggle.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                byte[] value = {};
                if(accelerationEnable!=null){
    ...
    private void displayData(byte[] data) {
        if (data != null) {
            accelerationEnable = data;
            updateTogleStage();
            if (data.length > 0) {
                final StringBuilder stringBuilder = new StringBuilder(data.length);
                for (byte byteChar : data)
                    stringBuilder.append(String.format("%02X ", byteChar));
                mDataField.setText(data.length + " " + stringBuilder.toString());
            }
        }
    }
    ...
    private void displayGattServices(List<BluetoothGattService> gattServices) {
        ...
        String uuid = null;
        accelerationEnableCharacteristic = null;
